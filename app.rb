...
class App
  ...
  def initialize
    @author = AuthorDataHandler.read
    @game = DataHandlerGame.read
  end
  ...
  def list_all_authors
    ...
    authors.each_with_index do |author, index|
      puts "#{index + 1}. #{author.firstname} #{author.lastname}"
    end
  end
  ...
  def add_a_game
    ...
    new_game = Game.new(nil, game_title, publish_date, false, num_players, last_played_date)
